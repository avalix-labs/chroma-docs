...
The good news for tests: **the wallet boundary doesn't change.** Whether the request is an `eth_sendTransaction` on Base or a Solana `signTransaction`, it surfaces as the same MetaMask popup — so the same Chroma methods (`authorize`, `confirm`, `reject`) drive it. This guide shows how to test that flow.
...
2. MetaMask shows a single connection popup → `metamask.authorize()`
3. A Solana action routes through `wallet_invokeMethod` and opens a signing popup → `metamask.confirm()`
...
There is no second prompt for Solana — that's the whole point of the CAIP-25 session. If your test was written for the old two-popup flow, drop the second `authorize()` call.

## Sign a Solana Transaction

A Solana action routes through `wallet_invokeMethod` with a `solana:*` scope. MetaMask shows a signing popup just like an EVM transaction, so you confirm it the same way:

```ts
// Your dApp builds a v0 transaction and asks MetaMask to sign it
await page.click('button:has-text("Send SOL")')

// Confirm the Solana signing request
...
<Callout type="info">
  EVM and Solana share a session but not an execution model. On EVM, MetaMask broadcasts the transaction. On Solana, MetaMask only **signs** — your dApp submits the signed bytes to an RPC itself. Assert against the wallet popup (`confirm`/`reject`), then assert your dApp's post-submit UI separately.
</Callout>
...
<Callout type="info">
  MetaMask uses `confirm()` and `reject()` — not the `approveTx()` / `rejectTx()` used by Chroma's Polkadot wallets. See the [MetaMask wallet reference](/docs/wallets/metamask) for the full method list.
</Callout>
...
- **Assert at the wallet boundary.** `confirm()` / `reject()` is what Chroma controls. What the transaction *does* afterward (broadcast on EVM, submit-to-RPC on Solana) is your dApp's code — assert its UI separately.
