...
<Callout type="info">
  MetaMask uses `approve()` and `reject()` instead of `approveTx()` and `rejectTx()` used by other Chroma wallets. The same `approve()` handles both the connection prompt and a transaction/signing prompt.
</Callout>
