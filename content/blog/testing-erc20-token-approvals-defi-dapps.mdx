...
test('approves ERC-20 and executes swap', async ({ page, wallets }) => {
  ...
  // Point MetaMask to the local Anvil node
  ...
  await metamask.authorize() // approve the wallet connection
...
- `addNetwork` configures MetaMask to use your Anvil node — no manual browser setup needed
- `metamask.authorize()` handles the connection approval popup automatically
- The first `metamask.confirm()` signs the `approve(spender, amount)` call
- `waitForSelector` confirms your dApp correctly listens for the allowance change before enabling the swap button
- The second `metamask.confirm()` signs the downstream action
